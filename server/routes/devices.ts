...
export const registerDevice: RequestHandler = async (req, res) => {
  ...
  await db.run('INSERT INTO devices (id,userId,platform,pushToken,lastSeenAt,createdAt) VALUES (?,?,?,?,?,?)', id, userId, data.platform || null, data.pushToken, now, now);
...
export const unregisterDevice: RequestHandler = async (req, res) => {
  ...
  const row = await db.get('SELECT * FROM devices WHERE id = ? AND userId = ?', id, userId);
...
export const listDevices: RequestHandler = async (req, res) => {
  const userId = req.params.userId;
  const rows = await db.all('SELECT id,platform,pushToken,lastSeenAt,createdAt FROM devices WHERE userId = ?', userId);
  res.json(rows);
};
