...
export const listNotifications: RequestHandler = async (req, res) => {
  ...
  const rows = await db.all('SELECT id,type,title,message,time,read,metadata,createdAt FROM notifications WHERE userId = ? ORDER BY createdAt DESC', userId);
...
export const createNotification: RequestHandler = async (req, res) => {
  ...
  await db.run('INSERT INTO notifications (id,userId,type,title,message,time,read,metadata,createdAt) VALUES (?,?,?,?,?,?,?,?,?)', id, userId, data.type || 'system', data.title, data.message || null, data.time || createdAt, data.read ? 1 : 0, data.metadata ? JSON.stringify(data.metadata) : null, createdAt);
  const row = await db.get('SELECT id,type,title,message,time,read,metadata,createdAt FROM notifications WHERE id = ?', id);
  res.status(201).json({
    id: row.id,
    type: row.type,
    title: row.title,
...
export const markAsRead: RequestHandler = async (req, res) => {
  ...
  res.json({ ...row, read: !!row.read, metadata: row.metadata ? row.metadata : undefined });
};
