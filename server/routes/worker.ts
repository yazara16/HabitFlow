...
async function computeStreakForHabit(
  habit: any,
  userId: string,
): Promise<number> {
  ...
  if (frequency === "weekly") {
    ...
    for (let i = 0; i < 365; i++) {
      ...
      const row = await db.get(
        "SELECT completedBoolean FROM habit_logs WHERE habitId = ? AND userId = ? AND date = ?",
        habitId,
        userId,
        iso,
      );
...
async function computeDailyStreak(
  ...
  const rows = await db.all(
    "SELECT date, completedBoolean FROM habit_logs WHERE habitId = ? AND userId = ? ORDER BY date DESC",
    habitId,
    userId,
  );
...
export async function computeAndRunWorker() {
  ...
  const achievements = await db.all(
    "SELECT id,key,title,description,criteria FROM achievements",
  );
  ...
  for (const u of users) {
    ...
    const habits = await db.all(
      "SELECT id,target,frequency,createdAt FROM habits WHERE userId = ?",
      userId,
    );
    ...
    for (const h of habits) {
      ...
      // Find today's log
      const todayLog = await db.get(
        "SELECT completedAmount, completedBoolean FROM habit_logs WHERE habitId = ? AND userId = ? AND date = ?",
        habitId,
        userId,
        todayIso,
      );
      ...
      if (streak !== undefined) {
        const lastCompletedRow = await db.get(
          "SELECT lastCompleted FROM habits WHERE id = ?",
          habitId,
        );
        const lastCompleted = completedToday
          ? todayIso
          : lastCompletedRow?.lastCompleted || null;
        await db.run(
          "UPDATE habits SET streak = ?, lastCompleted = COALESCE(?, lastCompleted) WHERE id = ?",
          streak,
          lastCompleted,
          habitId,
        );
      ...
      await db.run(
        "UPDATE habits SET completed = COALESCE(completed,0) WHERE id = ?",
        habitId,
      );
    }
    ...
    for (const a of achievements) {
      ...
      switch (criteria.type) {
        ...
        case "all_today": {
          ...
          const completedHabitsRow = await db.get(
            "SELECT COUNT(DISTINCT habitId) as c FROM habit_logs WHERE userId = ? AND date = ? AND completedBoolean = 1",
            userId,
            todayIso,
          );
        ...
        case "streak": {
          ...
          const rows = await db.all(
            "SELECT id,streak FROM habits WHERE userId = ?",
            userId,
          );
      ...
      if (shouldUnlock) {
        ...
        if (!exists) {
          ...
          await db.run(
            "INSERT INTO user_achievements (id,userId,achievementId,earnedAt,meta) VALUES (?,?,?,?,?)",
            uaId,
            userId,
            a.id,
            now,
          ...
          await db.run(
            ...
            "achievement",
            `Logro: ${a.key}`,
            `Has desbloqueado: ${a.title}`,
            now,
            0,
            JSON.stringify({ achievementKey: a.key }),
            now,
          );
