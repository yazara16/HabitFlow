...
export const getUserAchievements: RequestHandler = async (req, res) => {
  ...
  const rows = await db.all(
    "SELECT ua.id,ua.userId,ua.achievementId,ua.earnedAt,ua.meta,a.key,a.title,a.description FROM user_achievements ua JOIN achievements a ON a.id = ua.achievementId WHERE ua.userId = ? ORDER BY ua.earnedAt DESC",
    userId,
  );
  res.json(
    rows.map((r: any) => ({
...
export const unlockAchievement: RequestHandler = async (req, res) => {
  ...
  if (!achievementKey)
    return res.status(400).json({ message: "Missing achievementKey" });

  let ach = await db.get(
  ...
  if (!ach) {
    ...
    await db.run(
      "INSERT INTO achievements (id,key,title,description,criteria,createdAt) VALUES (?,?,?,?,?,?)",
      aid,
      achievementKey,
      achievementKey,
      achievementKey,
  ...
  await db.run(
    ...
    userId,
    "achievement",
    `Logro: ${achievementKey}`,
    `Has desbloqueado: ${achievementKey}`,
    now,
    0,
    JSON.stringify({ achievementKey }),
    now,
  );
...
export const seedCatalog: RequestHandler = async (_req, res) => {
  ...
  const seeds = [
    {
      ...
      description: "Completa 3 hábitos en el mismo día",
      criteria: JSON.stringify({ type: "count_per_day", count: 3 }),
    },
    {
      key: "perfect_day",
      title: "Día perfecto",
      description: "Completa todos tus hábitos hoy",
      criteria: JSON.stringify({ type: "all_today" }),
    },
    {
      key: "streak_7",
      title: "Racha 7+",
  ...
  for (const s of seeds) {
    ...
    await db.run(
      "INSERT INTO achievements (id,key,title,description,criteria,createdAt) VALUES (?,?,?,?,?,?) ON CONFLICT (key) DO NOTHING",
      uuidv4(),
      s.key,
      s.title,
      s.description,
