...
export const createReminder: RequestHandler = async (req, res) => {
  ...
  await db.run('INSERT INTO reminders (id,userId,habitId,timeOfDay,enabled,timezone,recurrence,days,nextRun,createdAt) VALUES (?,?,?,?,?,?,?,?,?,?)', id, userId, data.habitId || null, data.timeOfDay, data.enabled ? 1 : 0, data.timezone || null, data.recurrence || null, data.days ? JSON.stringify(data.days) : null, data.nextRun || null, now);
...
export const updateReminder: RequestHandler = async (req, res) => {
  ...
  const data = req.body || {};
  const row = await db.get('SELECT * FROM reminders WHERE id = ? AND userId = ?', id, userId);
  if (!row) return res.status(404).json({ message: 'Not found' });
  await db.run('UPDATE reminders SET timeOfDay = COALESCE(?, timeOfDay), enabled = COALESCE(?, enabled), timezone = COALESCE(?, timezone), recurrence = COALESCE(?, recurrence), days = COALESCE(?, days), nextRun = COALESCE(?, nextRun) WHERE id = ? AND userId = ?', data.timeOfDay || null, typeof data.enabled !== 'undefined' ? (data.enabled ? 1 : 0) : null, data.timezone || null, data.recurrence || null, data.days ? JSON.stringify(data.days) : null, data.nextRun || null, id, userId);
