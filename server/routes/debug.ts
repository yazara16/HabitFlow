...
export const listUsers: RequestHandler = async (_req, res) => {
  const rows = await db.all(
    "SELECT id,name,email,photoUrl,createdAt FROM users",
  );
  res.json(rows);
};
...
export const listHabits: RequestHandler = async (_req, res) => {
  const rows = await db.all(
    "SELECT id,userId,name,description,category,target,completed,streak,frequency,createdAt,lastCompleted FROM habits",
  );
  res.json(rows);
};
