...
export const createLog: RequestHandler = async (req, res) => {
  ...
  await db.run('INSERT INTO habit_logs (id,habitId,userId,date,completedAmount,completedBoolean,note,createdAt,updatedAt) VALUES (?,?,?,?,?,?,?,?,?)', id, habitId, userId, data.date, data.completedAmount || 0, data.completedBoolean ? 1 : 0, data.note || null, now, now);
...
export const updateLog: RequestHandler = async (req, res) => {
  ...
  const data = req.body || {};
  const row = await db.get('SELECT * FROM habit_logs WHERE id = ? AND habitId = ? AND userId = ?', logId, habitId, userId);
  ...
  await db.run('UPDATE habit_logs SET completedAmount = ?, completedBoolean = ?, note = ?, updatedAt = ? WHERE id = ?', completedAmount, completedBoolean, note, updatedAt, logId);
