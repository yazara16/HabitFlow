...
if (!JWT_SECRET) {
  ...
  console.warn('JWT_SECRET environment variable is not set. JWT signing and verification will be disabled until configured.');
}

export function signToken(payload: object) {
  if (!JWT_SECRET) throw new Error('JWT_SECRET is not configured. Set process.env.JWT_SECRET to sign tokens.');
