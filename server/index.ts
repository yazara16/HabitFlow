...
export function createServer() {
  ...
  app.use((req, res, next) => {
    ...
    res.setHeader('Content-Security-Policy', "default-src 'self'; img-src 'self' data: https://www.google-analytics.com https://www.googletagmanager.com; script-src 'self' 'unsafe-inline' https://www.google-analytics.com https://www.googletagmanager.com; style-src 'self' 'unsafe-inline'; frame-ancestors 'none'; base-uri 'self';");
  ...
  const requireOwner = (req: any, res: any, next: any) => {
    ...
    if (paramId && req.authUserId !== paramId) return res.status(403).json({ message: 'Forbidden' });
    next();
  };
