...
export function ensureCsrfCookie(req: any, res: any, next: any) {
  try {
    ...
    if (!token) {
      ...
      res.cookie('XSRF-TOKEN', token, { httpOnly: false, sameSite: 'lax', secure, path: '/', maxAge: 24*60*60*1000 });
    }
...
export function csrfProtection(req: any, res: any, next: any) {
  ...
  const headerToken = req.headers['x-xsrf-token'] || req.headers['x-xsrf-token'.toLowerCase()];

  // If server has no cookie token, allow (for compatibility)
  if (!cookieToken && !headerToken) return next();

  if (!cookieToken) return res.status(403).json({ message: 'CSRF token missing (cookie)' });
  if (!headerToken) return res.status(403).json({ message: 'CSRF token missing (header)' });
  if (String(cookieToken) !== String(headerToken)) return res.status(403).json({ message: 'CSRF token mismatch' });
